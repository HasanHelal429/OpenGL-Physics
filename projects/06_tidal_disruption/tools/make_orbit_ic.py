"""
Boost a relaxed star's center of mass onto a parabolic orbit around a fixed
black hole at the origin, so 06_tidal_disruption's blackhole gravity term
(src/kernels.hpp) can actually disrupt it. The star itself is untouched --
every particle's position/velocity gets the same rigid offset -- so this
only needs the star's shape (from tools/extract_ic_from_frame.py's settled
state), not its internal structure.

Orbit geometry (standard parabolic/Keplerian mechanics, e=1, specific
energy=0): pericenter r_p is set via the tidal radius r_t = R_star *
(M_bh/M_star)^(1/3) and a penetration factor beta = r_t/r_p (beta>1 ->
pericenter inside the tidal radius, real disruption expected; beta<1 ->
partial/no disruption). The star starts at separation r_0 = start_factor *
r_t, still approaching (before pericenter), in the given orbital plane.
...
def main():
    ...
    ap.add_argument("--start_factor", type=float, default=3.0, help="start distance, in units of r_t")
    ...
    print(f"M_bh={args.M_bh}  tidal radius r_t={r_t:.4f}  pericenter r_p={r_p:.4f}  "
          f"start r_0={r_0:.4f} ({args.start_factor}*r_t)")
