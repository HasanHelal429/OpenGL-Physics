"""
Exact solution of the classic 1D Euler-equation Riemann problem, ideal
Gamma-law gas -- the standard iterative pressure solver (Toro, "Riemann
Solvers and Numerical Methods for Fluid Dynamics", Ch. 4). This is the
reference solution decks/sod_shocktube.toml's HLLC finite-volume run is
validated against (see plot_shocktube.py). Identical in substance to
07_grhd/tools/exact_riemann_newtonian.py, which uses this same solver to
check that project's relativistic solver in its Newtonian limit.
"""
